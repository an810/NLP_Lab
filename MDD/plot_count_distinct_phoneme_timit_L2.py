...
for index, row in df.iterrows():
    canonical_phonemes = set([phoneme.replace('*', '') for phoneme in row['Canonical'].split()])
    transcript_phonemes = set([phoneme.replace('*', '') for phoneme in row['Transcript'].split()])
...
for index, row in df.iterrows():
    canonical_phonemes = [phoneme.replace('*', '') for phoneme in row['Canonical'].split()]
    transcript_phonemes = [phoneme.replace('*', '') for phoneme in row['Transcript'].split()]
...
# Sort the canonical and transcript phoneme counts in descending order
canonical_phoneme_counts = dict(sorted(canonical_phoneme_counts.items(), key=lambda item: item[1], reverse=True))
transcript_phoneme_counts = dict(sorted(transcript_phoneme_counts.items(), key=lambda item: item[1], reverse=True))
